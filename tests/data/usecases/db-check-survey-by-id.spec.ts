...
describe('DbCheckSurveyById', () => {
  ...
  test('Should call CheckSurveyByIdRepository with correct value', async () => {
    ...
    expect(checkSurveyByIdRepositorySpy.surveyId).toBe(surveyId)
  })

  test('Should return true of CheckSurveyByIdRepository returns true', async () => {
    const { sut } = makeSut()
    const exists = await sut.checkById(surveyId)
    expect(exists).toBe(true)
  })

  test('Should return false of CheckSurveyByIdRepository returns false', async () => {
    const { sut, checkSurveyByIdRepositorySpy } = makeSut()
  ...
  test('Should throw if CheckSurveyByIdRepository throws', async () => {
    const { sut, checkSurveyByIdRepositorySpy } = makeSut()
    jest.spyOn(checkSurveyByIdRepositorySpy, 'checkById').mockReturnValueOnce(Promise.reject(new Error()))
